    ...
    >
      ...
      <button
        ...
        className={`w-full py-3 rounded-md text-white font-semibold ${
          isLoading
            ? "bg-gray-400"
            : "bg-indigo-600 hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2"
        }`}
      >
        <span id="button-text">{isLoading ? <div className="spinner"></div> : "Pay now"}</span>
      </button>
      {message && (
        <div id="payment-message" className="mt-4 text-center text-sm text-red-600">
          {message}
        </div>
      )}
    </form>
